<template>
	<view>
		<view style="display: flex;flex-direction: row;width: 100%;height: 100%;align-items: center;">
			<view v-if="show_image" style="width: 41.5%;margin-left: 2.8%;height: 100%;padding: 40px 0 40px 0;display: flex;flex-direction: column;">
				<image src="../../static/login/bac.png" mode="widthFix" style="width: 100%;height: 100%;"></image>
			</view>
			
			<view :class="{'windows':show_image,'phone':!show_image}">
				<input disabled="true" style="height: 200rpx;" v-if="!show_image"/>
		...
		<view style="position: fixed;top: 40rpx;right: 50px;" v-if="show_image">
			<image src="../../static/logo.png" style="width: 130rpx;border-radius: 20upx;" mode="widthFix"></image>
		</view>
		<view style="position: fixed;top: 40rpx;left: 50px;" v-else>
			<image src="../../static/logo.png" style="width: 130rpx;border-radius: 20upx;" mode="widthFix"></image>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			send(){
				...
				uni.request({
					url:this.test_url+'/user/'+this.email,
					method:'GET',
					success: (res) => {
						if(res.code==0){
							uni.hideLoading();
			...
			login(){
				...
				uni.showLoading();
				uni.request({
					url:this.test_url+'/user/123/132',
					success: (res) => {
						console.log(res);
						uni.hideLoading();
					},
